...
import static org.doctorsmith.quartz.mongodb.Keys.*;
...
public class MongoDBJobStore implements JobStore, Constants {
    ...
    private DB selectDatabase(MongoClient mongo) {
    ...
    private MongoClient connectToMongoDB(final String mongoUriAsString) throws SchedulerConfigException {
        try {
            if(username == null || authDbName == null || password == null){
                ...
                MongoCredential credential = MongoCredential.createCredential(username, authDbName, password.toCharArray());
                List<MongoCredential> credentials = Arrays.asList(credential);
                return new MongoClient(new ServerAddress(mongoUriAsString), credentials);
            }
